...
namespace FOCA.GUI
{
    static class UpdateGUI
    {
                        ...
                        if (Program.data.domains != null)
                        {
                            try
                            {
                                ...
                                if (treeView.Nodes[TreeViewKeys.KProject.ToString()] != null)
                                {
                                    ...
                                    if (domainNode != null)
                                    {
                                        if (domainNode.IsExpanded || (domainNode.Nodes.Count == 0 && Program.data.domains.Items.Count > 0))
                                        {
                                            if (domains == null)
                                                domains = Program.data.GetDomains();

                                            NodeDomainsProcess(domains, domainNode);
                                        }
                                    }
                                }
                            }
        ...
        private static void NodeDomainsProcess(List<string> domains, TreeNode tnDomains)
        {
